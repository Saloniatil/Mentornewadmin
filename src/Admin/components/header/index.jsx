...
//    const profileImage = userData?.profileImage ? `${API_BASE_URL}/${userData.profileImage}` : avatar12; // Default avatar if no image
...
//                 Logout
//               </Link> */}
//               <Link className="dropdown-item" to="#"  onClick={handleLogout} > Logout  </Link> </div>
//           </li>
//           )}
//           {/* /User Menu */}
//         </ul>
...
const Header = () => {
  ...
  // Fetch user profile on mount & when auth status changes
  useEffect(() => {
    if (isAuthenticated) {
      fetchUserProfile();
    }
  ...
  const profileImage = userData?.profileImage ? `${API_BASE_URL}/${userData.profileImage}` : avatar12;
